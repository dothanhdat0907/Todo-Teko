...
export const AddTicket: React.FC<Props> = (props: Props) => {
  ...
  const setClass = (value: string) => {
    if (value === "A class") props.form.setFieldValue("price", "5.000.000");
    else if (value === "B class")
      props.form.setFieldValue("price", "3.000.000");
    else if (value === "C class")
      props.form.setFieldValue("price", "1.000.000");
    else props.form.setFieldValue("price", "10.000.000");
  };

  useEffect(() => {
    props.form.setFieldsValue({ ...props?.ticket });
  }, [props.ticket]);

  return (
    <Card title={props.action}>
      <Form form={props.form} initialValues={props.ticket} onFinish={onFinish}>
        <Form.Item
          name="name"
          label="Name"
